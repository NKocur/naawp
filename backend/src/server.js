...
app.get('/api/weddings/:weddingId/idea-boards/:boardId/attachments', async (request, reply) => {
  const user=await requireUser(request,reply);if(!user)return;const {weddingId,boardId}=request.params;await requireMembership(user.id,weddingId,allRoles);const result=await query('SELECT id,caption,source_url,original_name,content_type,byte_size,created_at,updated_at FROM idea_board_attachments WHERE wedding_id=$1 AND board_id=$2 AND archived_at IS NULL ORDER BY created_at',[weddingId,boardId]);return {attachments:result.rows};
});
