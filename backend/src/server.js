...
app.post('/api/weddings/:weddingId/imports/legacy-backup', async (request, reply) => {
  const user=await ownerUser(request,reply);if(!user)return;const {weddingId}=request.params;const body=z.object({backup:z.any()}).parse(request.body),data=body.backup?.data||body.backup||{},items=key=>Array.isArray(data[key])?data[key]:[],clean=value=>String(value||'').trim();
