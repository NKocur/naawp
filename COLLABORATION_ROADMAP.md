...
**Current implementation status:** In progress. The database foundation for shared budget categories, vendors, expense stages, payments, and repayment splits is in place. Server-side shared finance totals, budget-category read/create, shared expense list/create/edit/archive, payment list/create/edit/archive, and vendor list/create/edit/archive endpoints are available to Owners and Editors. The Budget page now displays those shared totals, lets Owners and Editors add, edit, or archive shared expenses and payments, and selects a payer from actual workspace members. The server verifies that any selected payer or reimbursement recipient belongs to the workspace. Real member reimbursement selection, budget-category management, and the Vendors screen still need their browser-local UI migrated to these shared records.
