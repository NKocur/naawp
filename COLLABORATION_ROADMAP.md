...
**UI consolidation status:** In progress. Retired local widgets are hidden after sign-in for migrated modules. The original Vendor and Idea Board card grids, Honeymoon hero and planning lists, Guest summary/table, and Day-of Contacts card now render shared records and route their actions to shared workflows. The remaining local-only sections are clearly labelled.
