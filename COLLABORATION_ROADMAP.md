...
**Idea boards status:** In progress. Shared board records and shared board attachments now support create, edit, open, and archive for Owners, Editors, and Contributors. Attachment files are stored in the Pi's persistent Docker upload volume with membership-checked access. Shared comments are the next migration step; legacy browser-local boards remain visibly separate until then.
